...
Процедура ПриСозданииНаСервере(Форма, МестоРазмещенияКоманд = Неопределено) Экспорт
	...
	УбратьИзСпискаКомандыПечатиСкрытыеФункциональнымиОпциями(КомандыПечати, Форма);
...
Процедура УбратьИзСпискаКомандыПечатиСкрытыеФункциональнымиОпциями(КомандыПечати, Форма)
	Для НомерКоманды = -КомандыПечати.Количество() + 1 По 0 Цикл
		...
		Для Каждого ФункциональнаяОпция Из ФункциональныеОпцииКомандыПечати Цикл
			Если Форма.ПолучитьФункциональнуюОпциюФормы(ФункциональнаяОпция) Тогда
				ВидимостьКоманды = Истина;
			КонецЕсли;
		КонецЦикла;
		Если Не ВидимостьКоманды Тогда
			КомандыПечати.Удалить(ОписаниеКомандыПечати);
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры

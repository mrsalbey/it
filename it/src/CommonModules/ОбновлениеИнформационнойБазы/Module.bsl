...
Процедура ПослеЗавершенияОбновления() Экспорт
	...
	ЗаписатьИнформациюОбОтложенномОбновлении(СведенияОбОбновлении);
...
Процедура ЗапланироватьОтложенноеОбновление(ИтерацииОбновления, НачальноеЗаполнениеДанных)
	...
	ПрошлоеЗначениеКонстанты = СведенияОбОбновленииИнформационнойБазы();
	
	ДеревоОбработчиков = Неопределено;
	ВыполнитьПроверкуДереваВыполненныхОбработчиковНаПрошлыеВерсии(ПрошлоеЗначениеКонстанты, ДеревоОбработчиков);
	
	СведенияОбОбновлении = Новый Структура();
	СформироватьСтруктуруКонстантыСведенияОбОбновлении(СведенияОбОбновлении, ДеревоОбработчиков, НачальноеЗаполнениеДанных);
...
Функция СведенияОбОбновленииИнформационнойБазы() Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() И Не ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
		...
		СведенияОбОбновлении = Новый Структура();
		ДеревоОбработчиков   = Неопределено;
		СформироватьСтруктуруКонстантыСведенияОбОбновлении(СведенияОбОбновлении, ДеревоОбработчиков, Истина);
...
// Только для внутреннего использования.
//
Процедура ВыполнитьПроверкуДереваВыполненныхОбработчиковНаПрошлыеВерсии(ПрошлоеЗначениеКонстанты, ДеревоОбработчиков)
	
	Если ПрошлоеЗначениеКонстанты <> Неопределено
		И ПрошлоеЗначениеКонстанты.ОтложенноеОбновлениеЗавершеноУспешно = Ложь Тогда
		
		// Оставляем только не выполненные
		НайденныеОбработчики = ПрошлоеЗначениеКонстанты.ДеревоОбработчиков.Строки.НайтиСтроки(Новый Структура("Статус", "Выполнено"), Истина);
		...
		НайденныеОбработчики = ПрошлоеЗначениеКонстанты.ДеревоОбработчиков.Строки.НайтиСтроки(Новый Структура("Статус", "Ошибка"), Истина);
		Для Каждого ЭлементДерева Из НайденныеОбработчики Цикл
			ЭлементДерева.ЧислоПопыток = 0;
		КонецЦикла;
		ДеревоОбработчиков = ПрошлоеЗначениеКонстанты.ДеревоОбработчиков;
...
// Записывает обновленные данные по отложенному обновлению в константу "СведенияОбОбновлении".
//
Процедура ЗаписатьИнформациюОбОтложенномОбновлении(СведенияОбОбновлении)
	...
	Для Каждого ДеревоОбработчиковБиблиотека Из СведенияОбОбновлении.ДеревоОбработчиков.Строки Цикл
		...
		Для Каждого ДеревоОбработчиковВерсия Из ДеревоОбработчиковБиблиотека.Строки Цикл
			...
			Если Не ОбработчикиВыполнялись И ЕстьНеудачные И Не ВыполнитьНеудачные Тогда
				ВыполнитьОтложенныйОбработчикОбновления(СведенияОбОбновлении, Истина);
			КонецЕсли;
...
// Только для внутреннего использования
//
Процедура СформироватьСтруктуруКонстантыСведенияОбОбновлении(СведенияОбОбновлении, ДеревоОбработчиков, НачальноеЗаполнениеДанных)
	...
	СведенияОбОбновлении.Вставить("ЛегальностьПолученияОбновленияПодтверждена", НачальноеЗаполнениеДанных);
	СведенияОбОбновлении.Вставить("ДеревоОбработчиков");
	
	Если ДеревоОбработчиков = Неопределено Тогда

...
Процедура ВыполнитьОткрытиеФормыОбработки()
	...
	ПоказатьРезультатВыполненияОбработки(Неопределено, Истина);
КонецПроцедуры
...
Процедура ВыполнитьКлиентскийМетодОбработки()
	...
	#Иначе
		...
		Иначе
			ФормаОбработки = ПолучитьФорму("ВнешняяОбработка."+ ИмяОбработки +".Форма", ПараметрыОбработки, ВладелецФормы);
		КонецЕсли;
	#КонецЕсли
...
Процедура ВыполнитьОткрытиеПечатнойФормы()
	...
	Если Не Отказ Тогда
		...
		ОткрытьФорму("ОбщаяФорма.ПечатьДокументов", ПараметрыОткрытия, ВладелецФормы);
	КонецЕсли;

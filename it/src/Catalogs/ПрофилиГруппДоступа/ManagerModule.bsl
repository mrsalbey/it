...
Функция ПредопределенныеПрофили() Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено()
	   И НЕ ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
		...
		Пока Выборка.Следующий() Цикл
			ИмяПредопределенныхДанных = ПолучитьИмяПредопределенного(Выборка.Ссылка);
...
Функция ОбновитьПрофильГруппДоступа(СвойстваПрофиля, НеОбновлятьРолиПользователей = Ложь)
	...
	Если ПрофильСсылка = Неопределено Тогда
		...
		Если ЗначениеЗаполнено(СвойстваПрофиля.Имя) Тогда
			...
			Пока Выборка.Следующий() Цикл
				Если ВРег(СвойстваПрофиля.Имя) = ВРег(ПолучитьИмяПредопределенного(Выборка.Ссылка)) Тогда

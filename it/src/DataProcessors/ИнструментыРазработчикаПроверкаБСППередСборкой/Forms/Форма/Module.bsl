...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДобавитьПроверку(НСтр("ru = 'Модули обычного и управляемого приложения должны совпадать'"), "Подключаемый_ПроверитьСовпадениеМодулейУправляемогоИОбычногоПриложений()", "Требуется закрытие конфигуратора!");
	...
	ДобавитьПроверку(НСтр("ru = 'Демонстрационные примеры должны быть правильно закомментированы'"), "Подключаемый_ПроверитьКорректностьКомментированияДемоПримеров()", "Требуется закрытие конфигуратора!");
	...
	ДобавитьПроверку(НСтр("ru = 'Некорректны вызовы процедур и функций общего модуля из него самого, содержащие имя модуля.'"), "Подключаемый_ПроверитьВызовыОбщихМодулейИзНихСамих()", "Требуется закрытие конфигуратора!");
	ДобавитьПроверку(НСтр("ru = 'Использование экспортных процедур в модулях форм, общих и локальных команд недопустимо.'"), "Подключаемый_ПроверитьЭкспортностьМодулейКомандИФорм()", "Требуется закрытие конфигуратора!");
	...
	ДобавитьПроверку(НСтр("ru = 'Комментарии блоков кода, относящихся к той или иной подсистемы должны быть корректными'"), "Подключаемый_ПроверитьКомментарииДляБлоковКодаПодсистем()", "Требуется закрытие конфигуратора!");
...
&НаКлиенте
Функция Подключаемый_ПроверитьЭкспортностьМодулейКомандИФорм()
	...
	МассивМодулей = НайтиФайлы(КаталогВыгрузкиМодулей, "*.txt");
	Для Каждого ФайлМодуля Из МассивМодулей Цикл
		Если НЕ (Прав(ФайлМодуля.ИмяБезРасширения, 13) = "МодульКоманды" ИЛИ Прав(ФайлМодуля.ИмяБезРасширения, 12) = "Форма.Модуль") Тогда
			Продолжить;
		КонецЕсли;
		...
		Пока НомерТекущейСтроки < КоличествоСтрок Цикл
			...
			СтрокаПроверки = ТекстФайла.ПолучитьСтроку(НомерТекущейСтроки);
			ОбъявлениеПроцедуры = Найти(СокрЛП(СтрокаПроверки), "Процедура") = 1 ;
			ОбъявлениеФункции = Найти(СокрЛП(СтрокаПроверки), "Функция") = 1;
			ЭкспортностьВКонце = Найти(СтрокаПроверки, " Экспорт") > 0 И СтрДлина(Сред(СокрЛП(СтрокаПроверки), Найти(СтрокаПроверки, " Экспорт"))) = СтрДлина(" Экспорт");
			
			УстановкаПараметров = Найти(СтрокаПроверки, "УстановитьПараметрыИспользования")> 0;
			АвтоТест = Найти(СтрокаПроверки, "АвтоТест_")> 0;
			
			Если (ОбъявлениеПроцедуры  ИЛИ ОбъявлениеФункции) И ЭкспортностьВКонце И НЕ УстановкаПараметров И НЕ АвтоТест Тогда
				СтрокаОшибки = "%1 : %2";
				СтрокаОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаОшибки,
					ФайлМодуля.ИмяБезРасширения,
					"Строка " + НомерТекущейСтроки);
	...
	ОписаниеИсправления = НСтр("ru = 'В модулях форм и команд недопустимо использовать экспортные процедуры и функции'");
	ПредставлениеОшибки = НСтр("ru = 'Экспортная процедура или функция в модуле формы (команды)'");
	СтруктураОшибок =  Новый Структура("ПредставлениеОшибки, ОписаниеИсправления, ОшибочныеДанные", 
		ПредставлениеОшибки, ОписаниеИсправления, СписокОшибочныхОбъектов);
	Возврат СтруктураОшибок;
...
Функция Подключаемый_ПроверитьРолиНаНаличиеНедопустимыхПрав()
	...
	НеразделенныеДанныеДоступныеДляИзменения = ПользователиСлужебныйПовтИсп.Параметры().НеразделенныеДанныеДоступныеДляИзменения;
	...
	Для каждого Роль Из Метаданные.Роли Цикл
		...
		ПраваИзмененияОбъектов = НеразделенныеДанныеДоступныеДляИзменения.Получить(ИмяРоли);
		
		Если ПраваИзмененияОбъектов = Неопределено Тогда
			Продолжить;
		КонецЕсли;
